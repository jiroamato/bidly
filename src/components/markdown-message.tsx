...
import React, { useState, useCallback, useRef, useEffect, type ComponentPropsWithoutRef } from "react";
...
// Extracted to module-level constant — never recreated between renders
const REMARK_PLUGINS = [remarkGfm];

const MARKDOWN_COMPONENTS = {
  h1: ({ children, ...props }: ComponentPropsWithoutRef<"h2"> & { children?: React.ReactNode }) => (
    <h2 className="text-base font-semibold mt-4 mb-2" style={{ color: "var(--text-primary)" }} {...props}>
      {children}
    </h2>
  ),
  ...
  p: ({ children, ...props }: ComponentPropsWithoutRef<"p"> & { children?: React.ReactNode }) => (
    <p className="mb-3 leading-[1.7]" {...props}>{children}</p>
  ),
  ...
  pre: ({ children, ...props }: ComponentPropsWithoutRef<"pre"> & { children?: React.ReactNode }) => {
  ...
  table: ({ children, ...props }: ComponentPropsWithoutRef<"table"> & { children?: React.ReactNode }) => (
    <div className="mb-3 overflow-x-auto">
      <table className="w-full text-[12px]" style={{ borderCollapse: "collapse" }} {...props}>
        {children}
      </table>
    </div>
  ),
  th: ({ children, ...props }: ComponentPropsWithoutRef<"th"> & { children?: React.ReactNode }) => (
    <th
      ...
      style={{ borderBottom: "2px solid var(--bidly-border, #e0e0e0)", fontFamily: "var(--font-mono)" }}
      {...props}
    >
      {children}
    </th>
  ...
  td: ({ children, ...props }: ComponentPropsWithoutRef<"td"> & { children?: React.ReactNode }) => (
    <td className="px-3 py-2" style={{ borderBottom: "1px solid var(--border-light, #e0e0e0)" }} {...props}>
      {children}
    </td>
  ),
  ...
  hr: (props: ComponentPropsWithoutRef<"hr">) => (
    <hr className="my-4" style={{ borderColor: "var(--border-light, #e0e0e0)" }} {...props} />
  ),
} as const;
 ...
 *
 * While streaming, renders plain text (whitespace-preserving) to avoid
 * re-parsing the entire accumulated markdown AST on every token (~O(n²)).
 * Once streaming ends, switches to full ReactMarkdown for final render.
 */
export const MarkdownMessage = React.memo(function MarkdownMessage({
  content,
  isStreaming = false,
}: MarkdownMessageProps) {
  // Track whether this message has EVER streamed — used to trigger the
  // one-time switch from plain text to parsed markdown after streaming ends.
  const wasStreamingRef = useRef(false);
  const [showParsed, setShowParsed] = useState(!isStreaming);
  ...
  return (
    <div className="markdown-message">
      {showParsed ? (
        <ReactMarkdown remarkPlugins={REMARK_PLUGINS} components={MARKDOWN_COMPONENTS as any}>
          {content}
        </ReactMarkdown>
      ) : (
